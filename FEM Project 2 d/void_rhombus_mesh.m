function [NL, EL] = void_rhombus_mesh(d1, d2, p, m, c, element_type)
   ...
   inc = 0;
    ...
    for i = 1:p+1
        ...
        if ((d1-(c/sqrt(2)))/2)+(i-1)*(c/p) <= d1/2
            coor11(m*(p+1)+i,1) = ((d1-(c/sqrt(2)))/2)+(i-1)*(c/p);
            coor11(m*(p+1)+i,2) = ((d2-(c/sqrt(2)))/2)-(i-1)*(c/p);
            g=g+1;
        else
            coor11(m*(p+1)+i,1) = 1.02*((d1-(c/sqrt(2)))/2)+(i-1)*(c/p);
            coor11(m*(p+1)+i,2) = 1.05*((d2-(2*c/sqrt(2)))/2)+(i-1-g)*(c/p);
        end
    end
    for i = 1:m-1
...
alp=1;
    for i = 1:p+1
        ...
        if ((d1-(c/sqrt(2)))/2)+(i-1)*(c/p) <= d1/2
            coor22(m*(p+1)+i,1) = ((d1-(c/sqrt(2)))/2)+(i-1)*(c/p);
            coor22(m*(p+1)+i,2) = ((d2+(c/sqrt(2)))/2)+(i-1)*(c/p);
            alp=alp+1;
        else
            coor22(m*(p+1)+i,1) = 1.03*((d1-(c/sqrt(2)))/2)+(i-1)*(c/p);
            coor22(m*(p+1)+i,2) = 0.95*((d2+(2*c/sqrt(2)))/2)-(i-1-alp)*(c/p);
        end
    end
...
fatih=1;
    for i = 1:p-1
        ...
        if ((d2-(c/sqrt(2)))/2)+(i-1)*(c/p) <= d2/2
            coor33(m*(p-1)+i,1) = ((d1-(c/sqrt(2)))/2)-c/(1.5*p)-(i-1)*(c/(1.5*p));
            coor33(m*(p-1)+i,2) = ((d2-(c/sqrt(2)))/2)+c/(1.5*p)+(i-1)*(c/(1.5*p));
            fatih=fatih+1;
        else
            coor33(m*(p-1)+i,1) = ((d1-(c/sqrt(2)))/2)+0.01+(i-1-fatih)*(c/p);
            coor33(m*(p-1)+i,2) = ((d2-(c/sqrt(2)))/2)+(i-1)*(c/p);
...
terim=1;
    for i = 1:p-1
        ...
        if ((d2-(c/sqrt(2)))/2)+(i-1)*(c/p) <= d2/2
            coor44(m*(p-1)+i,1) = ((d1+(2*c/sqrt(2)))/2)+(c/(1.5*p))+(i-1)*(c/(1.5*p));
            coor44(m*(p-1)+i,2) = ((d2-(c/sqrt(2)))/2)+(c/(1.5*p))+(i-1)*(c/(1.5*p));
            terim=terim+1;
        else
            coor44(m*(p-1)+i,1) = ((d1+(2*c/sqrt(2)))/2)-0.01-(i-1-terim)*(c/p);
            coor44(m*(p-1)+i,2) = ((d2-(c/sqrt(2)))/2)+(i-1)*(c/p);
